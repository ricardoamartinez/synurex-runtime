...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SYNUREX_SKIP_CHANNELS;
    prevSkipGmail = process.env.SYNUREX_SKIP_GMAIL_WATCHER;
    process.env.SYNUREX_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SYNUREX_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SYNUREX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SYNUREX_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
