...
describe("gateway config.patch", () => {
  it("merges patches without clobbering unrelated config", async () => {
    ...
    expect(get2Res.payload?.config?.channels?.telegram?.botToken).toBe("__SYNUREX_REDACTED__");
  ...
  it("preserves credentials on config.set when raw contains redacted sentinels", async () => {
    ...
    expect(rawRedacted).toContain("__SYNUREX_REDACTED__");
  ...
  it("writes config, stores sentinel, and schedules restart", async () => {
    ...
    const sentinelPath = path.join(os.homedir(), ".synurex", "restart-sentinel.json");
...
describe("gateway server sessions", () => {
  it("filters sessions by agentId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Synurex-sessions-agents-"));
  ...
  it("resolves and patches main alias to default agent main key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Synurex-sessions-"));
