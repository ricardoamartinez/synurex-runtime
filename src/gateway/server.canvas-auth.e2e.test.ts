...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.SYNUREX_CONFIG_PATH;
  const prevDisableCache = process.env.SYNUREX_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "Synurex-canvas-auth-test-"));
  const configPath = path.join(dir, "synurex.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.SYNUREX_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.SYNUREX_DISABLE_CONFIG_CACHE;
    } else {
      process.env.SYNUREX_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
