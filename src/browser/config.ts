import type { BrowserConfig, BrowserProfileConfig, SynurexConfig } from "../config/config.js";
...
import {
  DEFAULT_SYNUREX_BROWSER_COLOR,
  DEFAULT_SYNUREX_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_SYNUREX_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_SYNUREX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "synurex" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SYNUREX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SYNUREX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Synurex browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for Synurex/Synurex CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SynurexConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SYNUREX_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "synurex";
