...
import type { SynurexConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SynurexConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SynurexConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SynurexConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SynurexConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SynurexConfig)),
    ),
  };
}
