...
const ETC_SYNUREX_DIR = path.join(ROOT_DIR, "etc", "synurex");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "synurex.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSynurexPath(...parts: string[]) {
  return path.join(ETC_SYNUREX_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSynurexPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "synurex.json"))).toEqual({
      shared: true,
    });
  });
});
