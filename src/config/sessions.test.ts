...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-Synurex",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-Synurex#general");
  });
  ...
  it("derives session transcripts dir from SYNUREX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SYNUREX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.SYNUREX_STATE_DIR = "/custom/state";
    try {
