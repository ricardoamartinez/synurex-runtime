...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SYNUREX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*Synurex-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "synurex";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "synurex");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "Synurex-gw");
  ...
  records.push(`_Synurex-gw._tcp IN PTR ${instanceLabel}._Synurex-gw._tcp`);
  records.push(`${instanceLabel}._Synurex-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._Synurex-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; Synurex-content-hash: ${contentHash}\n${contentBody}`;
}
