...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.SYNUREX_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.SYNUREX_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.SYNUREX_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[Synurex] ${parsed.error}`);
    process.exit(2);
  }
