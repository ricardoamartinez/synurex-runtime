...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Synurex-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.SYNUREX_STATE_DIR;
    prevBundledDir = process.env.SYNUREX_BUNDLED_HOOKS_DIR;
    process.env.SYNUREX_STATE_DIR = path.join(baseDir, "state");
    process.env.SYNUREX_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.SYNUREX_BUNDLED_HOOKS_DIR;
    } else {
      process.env.SYNUREX_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          Synurex: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
