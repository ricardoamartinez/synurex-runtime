...
describe("createSynurexCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createSynurexCodingTools({
      config,
    ...
    const anthropicTools = createSynurexCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createSynurexCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createSynurexCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
