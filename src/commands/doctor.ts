...
function resolveMode(cfg: SynurexConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: SynurexConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("synurex configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("synurex config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("synurex setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("synurex doctor --fix")}" to apply changes.`);
  }
