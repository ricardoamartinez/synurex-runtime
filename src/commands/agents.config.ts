...
export function listAgentEntries(cfg: SynurexConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SynurexConfig, agentId: string) {
...
function resolveAgentModel(cfg: SynurexConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SynurexConfig): AgentSummary[] {
