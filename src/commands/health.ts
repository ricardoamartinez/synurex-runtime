...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SYNUREX_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: SynurexConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SYNUREX_DEBUG_HEALTH);
    if (opts.verbose) {
