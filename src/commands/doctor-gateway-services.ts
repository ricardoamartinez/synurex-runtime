...
export async function maybeRepairGatewayServiceConfig(
  ...
  const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
    env: process.env,
    port,
    token: cfg.gateway?.auth?.token ?? (process.env.SYNUREX_GATEWAY_TOKEN ?? process.env.SYNUREX_GATEWAY_TOKEN),
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (moltbot) now?",
      initialValue: true,
    });
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Synurex gateway next.");
      }
    }
  }
