...
function captureEnv(): EnvSnapshot {
  return {
    ...
    disableConfigCache: process.env.SYNUREX_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("SYNUREX_STATE_DIR", prev.stateDir);
  restoreEnvVar("SYNUREX_CONFIG_PATH", prev.configPath);
  restoreEnvVar("SYNUREX_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("SYNUREX_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("SYNUREX_SKIP_CRON", prev.skipCron);
  restoreEnvVar("SYNUREX_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("SYNUREX_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("SYNUREX_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("SYNUREX_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.SYNUREX_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "synurex.json");
  process.env.HOME = tempHome;
  process.env.SYNUREX_STATE_DIR = tempHome;
  process.env.SYNUREX_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("Synurex-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("Synurex-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("Synurex-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("Synurex-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("Synurex-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("Synurex-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
