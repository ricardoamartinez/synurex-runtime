---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Synurex deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Synurex cron scheduling for periodic checks, or version status checks on a machine running Synurex (laptop, workstation, Pi, VPS).
---

# Synurex Host Hardening

## Overview

Assess and harden the host running Synurex, then align it to a user-defined risk tolerance without breaking access. Use Synurex security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Synurex changes the host firewall, SSH, or OS updates; it does not.
...
5. Synurex gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Synurex can access sensitive data.
...
### 2) Run Synurex security audits (read-only)

As part of the default read-only checks, run `Synurex security audit --deep`. Only offer alternatives if the user requests them:

1. `Synurex security audit` (faster, non-probing)
2. `Synurex security audit --json` (structured output)

Offer to apply Synurex safe defaults (numbered):

1. `Synurex security audit --fix`

Be explicit that `--fix` only tightens Synurex defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Synurex version/update status (read-only)

As part of the default read-only checks, run `Synurex update status`.
...
- Credential hygiene notes (location of Synurex creds, prefer disk encryption)
...
After Synurex install or first hardening pass, run at least one baseline audit and version check:

- `Synurex security audit`
- `Synurex security audit --deep`
- `Synurex update status`

Ongoing monitoring is recommended. Use the Synurex cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `Synurex cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `Synurex update status`
...
Before creating, `Synurex cron list` and match on exact `name`. If found, `Synurex cron edit <id> ...`.
If not found, `Synurex cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `Synurex update status` (preferred for source checkouts and channels)
2. `npm view Synurex version` (published npm version)

## Synurex command accuracy

Use only supported commands and flags:

- `Synurex security audit [--deep] [--fix] [--json]`
- `Synurex status` / `Synurex status --deep`
- `Synurex health --json`
- `Synurex update status`
- `Synurex cron add|list|runs|run`

Do not invent CLI flags or imply Synurex enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Synurex compaction:
