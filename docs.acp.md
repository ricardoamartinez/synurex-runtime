...
This document describes how the Synurex ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`Synurex acp` exposes an ACP agent over stdio and forwards prompts to a running
Synurex Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `Synurex acp` and speaks ACP messages over stdio.
...
`Synurex acp` resolves the Gateway URL and auth from CLI flags or config:
