...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `Synurex.internal.`), **or**
...
The Gateway advertises `_Synurex-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `Synurex.internal.`) and Tailscale split DNS.
...
- The Gateway canvas host serves `/__SYNUREX__/canvas/` and `/__SYNUREX__/a2ui/`.
...
- Pairing prompt never appears: run `Synurex nodes pending` and approve manually.
