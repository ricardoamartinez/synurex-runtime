---
summary: "Chrome extension: let Synurex drive your existing Chrome tab"
read_when:
...
The Synurex Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate Synurex-managed Chrome profile.
...
Synurex then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Synurex release (npm package) as static files. There is no separate “build” step.

After upgrading Synurex:

- Re-run `Synurex browser extension install` to refresh the installed files under your Synurex state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Synurex ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Synurex to control.
...
- `ON`: attached; Synurex can drive that tab.
...
Debugging: `Synurex sandbox explain`
...
`Synurex browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `Synurex browser extension install` first to copy the extension to a stable location under your Synurex state directory.
...
- **This is not isolated** like the dedicated Synurex-managed profile.
