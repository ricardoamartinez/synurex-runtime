---
summary: "Use MiniMax M2.1 in Synurex"
read_when:
  - You want MiniMax models in Synurex
...
```bash
Synurex plugins enable minimax-portal-auth  # skip if already loaded.
Synurex gateway restart  # restart if gateway is already running
Synurex onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/Synurex/Synurex/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `synurex.json`:
...
- Use `Synurex models list` and `Synurex models set minimax/MiniMax-M2.1` to switch.
...
- Running `Synurex configure` and selecting **MiniMax M2.1**, or
