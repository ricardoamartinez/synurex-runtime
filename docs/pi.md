...
This document describes how Synurex integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Synurex uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Synurex directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Synurex's local TUI mode)                                             |
...
├── pi-tools.ts                    # createSynurexCodingTools()
...
├── Synurex-tools.ts              # Synurex-specific tools
...
2. **Custom Replacements**: Synurex replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Synurex Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Synurex's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Synurex CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Synurex wraps this with `guardSessionManager()` for tool result safety.
...
Synurex maintains an auth profile store with multiple API keys per provider:
...
Synurex loads custom pi extensions for specialized behavior:
...
Synurex also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Synurex Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Synurex tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.synurex/agents/<agentId>/sessions/` (or `$SYNUREX_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-Synurex-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-Synurex-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-Synurex-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-Synurex-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
