...
- Jobs persist under `~/.synurex/cron/` so restarts don’t lose schedules.
...
Synurex cron list
Synurex cron run <job-id>
Synurex cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.synurex/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `Synurex cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Synurex defaults to `announce`.
...
- Job store: `~/.synurex/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.synurex/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
Synurex cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
Synurex cron edit <jobId> --agent ops
Synurex cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
Synurex cron run <jobId>
Synurex cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
Synurex cron edit <jobId> \
...
```bash
Synurex system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`Synurex system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SYNUREX_SKIP_CRON`.
