...
Synurex integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Synurex owns lifecycle when `channels.signal.autoStart=true`.
...
- Synurex spawns `imsg rpc` as a child process (legacy iMessage integration).
