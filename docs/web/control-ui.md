...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/Synurex`)
...
If the page fails to load, start the Gateway first: `Synurex gateway`.
...
you revoke it with `Synurex devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.synurex/synurex.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Synurex
...
Synurex **blocks** Control UI connections without device identity.
