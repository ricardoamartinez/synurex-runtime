...
```bash
Synurex tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `Synurex tui --deliver`
...
- Check the Gateway logs: `Synurex logs --follow`.
- Confirm the agent can run: `Synurex status` and `Synurex models status`.
...
- No agents in picker: check `Synurex agents list` and your routing config.
