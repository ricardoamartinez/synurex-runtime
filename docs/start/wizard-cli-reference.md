...
read_when:
  - You need detailed behavior for Synurex onboard
...
This page is the full reference for `Synurex onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.synurex/synurex.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `Synurex doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `Synurex pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `Synurex health`.
    - `Synurex status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.synurex/.env` so launchd can read it.
...
- OAuth credentials: `~/.synurex/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.synurex/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`Synurex onboard`](/cli/onboard)
