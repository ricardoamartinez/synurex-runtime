...
## Quick check: `synurex security audit`
...
- Tighten local perms (`~/.Synurex` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

Synurex is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Synurex also attempts a best-effort live Gateway probe.
...
`synurex security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${Synurex_GATEWAY_PASSWORD}
```
...
Synurex stores session transcripts on disk under `~/.synurex/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.Synurex` (see the audit section below). If you need
...
Synurex can refresh the skills list mid-session:
...
- If you install plugins from npm (`synurex plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.synurex/extensions/<pluginId>/` (or `$Synurex_STATE_DIR/extensions/<pluginId>/`).
  - Synurex uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
Synurex pairing approve <channel> <code>
```
...
By default, Synurex routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Synurex has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.synurex/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.Synurex or your logs.”
...
4. **Re-run audit**
   - `synurex security audit --deep` and confirm the report is clean.
...
- `~/.synurex/synurex.json`: `600` (user read/write only)
- `~/.Synurex`: `700` (user only)

`synurex doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `Synurex_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_Synurex-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `Synurex_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `synurex doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `Synurex_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `Synurex_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Synurex
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Synurex verifies the identity by resolving the
...
- Synurex will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.synurex/` (or `$Synurex_STATE_DIR/`) may contain secrets or private data:

- `Synurex.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `synurex status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@Synurex", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.synurex/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `Synurex` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Synurex clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `synurex gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `Synurex_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/Synurex/Synurex-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.synurex/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Synurex version
...
Found a vulnerability in Synurex? Please report responsibly:

1. Email: [security@synurex.com](mailto:security@synurex.com)
