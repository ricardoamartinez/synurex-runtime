...
- During heartbeat runs, Synurex treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `Synurex sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Synurex skips the heartbeat run entirely (no model call).
...
```bash
Synurex system event --text "Check for urgent follow-ups" --mode now
```
