...
```bash
Synurex plugins install @Synurex/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
Synurex plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Synurex" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `Synurex pairing list nextcloud-talk`
  - `Synurex pairing approve nextcloud-talk <CODE>`
