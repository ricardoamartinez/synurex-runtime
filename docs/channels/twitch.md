...
read_when:
  - Setting up Twitch chat integration for Synurex
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Synurex connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
Synurex plugins install @Synurex/twitch
```

Local checkout (when running from a git repo):

```bash
Synurex plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SYNUREX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "Synurex", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SYNUREX_TWITCH_ACCESS_TOKEN env var)
...
```bash
SYNUREX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
