...
Tlon is a decentralized messenger built on Urbit. Synurex connects to your Urbit ship and can
...
```bash
Synurex plugins install @Synurex/tlon
```

Local checkout (when running from a git repo):

```bash
Synurex plugins install ./extensions/tlon
```
...
Use these with `Synurex message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Synurex replies in-thread.
