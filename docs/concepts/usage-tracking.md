...
- `/usage cost` in chats: local cost summary aggregated from Synurex session logs.
- CLI: `Synurex status --usage` prints a full per-provider breakdown.
- CLI: `Synurex channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
