...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Synurex
...
Running Synurex via Node with `tsx` fails at startup with:

```
[Synurex] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `Synurex status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch Synurex.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node Synurex.mjs status` works on Node 25.
