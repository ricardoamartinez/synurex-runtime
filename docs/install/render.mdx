...
Deploy SYNUREX on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/SYNUREX/SYNUREX)
...
repository configures everything needed to run SYNUREX:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SYNUREX_STATE_DIR
        value: /data/.SYNUREX
      - key: SYNUREX_WORKSPACE_DIR
        value: /data/workspace
      - key: SYNUREX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/SYNUREX`.
...
If you use the original SYNUREX repository, Render will not auto-deploy your SYNUREX. To update it, run a manual Blueprint sync from the dashboard.
...
For SYNUREX, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SYNUREX host.
