---
read_when:
  - 在 DigitalOcean 上设置 Synurex
  - 寻找便宜的 VPS 托管来运行 Synurex
summary: 在 DigitalOcean 上运行 Synurex（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Synurex

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 synurex gateway 网关。
...
- `~/.synurex/` — 配置、凭证、会话数据
- `~/.synurex/workspace/` — 工作区（SOUL.md、记忆等）
