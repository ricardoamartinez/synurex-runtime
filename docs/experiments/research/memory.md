...
read_when:
  - Designing workspace memory (~/.synurex/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Synurex integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.synurex/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.synurex/workspace` as canonical human-readable memory.
...
```
~/.synurex/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Synurex**, but keep a separable core library.

### Why integrate into Synurex?

- Synurex already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `Synurex memory recall "…" --k 25 --since 30d`
  - `Synurex memory reflect --since 7d`
...
Pragmatic take for `~/.synurex/workspace`:
