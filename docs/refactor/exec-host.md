...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.synurex/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.synurex/node.json` (node id + pairing token).
...
Path: `~/.synurex/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.synurex/exec-approvals.sock",
...
- Unix socket at `~/.synurex/exec-approvals.sock` (0600).
