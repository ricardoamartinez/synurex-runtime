...
// NOTE: This extension is intended to be bundled with Synurex.
// When running from source (tests/dev), Synurex internals live under src/.
...
export function createLlmTaskTool(api: SynurexPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via Synurex.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "Synurex-llm-task-"));
